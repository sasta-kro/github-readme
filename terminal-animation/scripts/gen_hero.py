#!/usr/bin/env python3
"""Render configurable profile hero assets from the sources in assets/.

The rotating crest renderer is adapted from syamxm's profile README. Profile
copy, colours, and animation timing live in config/profile.toml.
...
def load_config():
    with open(PROFILE_CONFIG, "rb") as handle:
...
def build_spinner(model, colour, title, frame_count, loop_seconds):
    points, scale, rows, y_unit = model
...
def build_ticker(messages, title, duration, theme):
...
def main():
    ...
    light_theme = config["theme"]["light"]

    model = build_model(read_art("ascii-art.txt"))
    display_name = profile["display_name"]
    wordmark = trim_lines(
        Figlet(font=wordmark_config["font"], width=160).renderText(display_name)
    )
    spinner_args = (
        spinner_config["frame_count"],
        spinner_config["loop_seconds"],
    )
    wordmark_args = (
    ...
    write(
        "ticker.svg",
        build_ticker(
            ticker_config["messages"],
            "Profile status ticker",
            ticker_config["scroll_seconds"],
            dark_theme,
        ),
    )
